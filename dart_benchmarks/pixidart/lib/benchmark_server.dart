...
import 'package:pixi_dart_benchmark/benchmarks/keys.dart';
...
class _Server {
  ...
  static _removeXFrameOptionsHandler(Handler handler) {
    return (Request request) async {
      ...
      _logger.info("Respon has ${response.headers.length} headers");

//      response.headers.putIfAbsent('x-frame-options', '')
//      response.headers.forEach((k,v) => _logger.info("$k -- $v"));
//
//      var headers = new Map.from(response.headers);
//      headers.addAll({"-frame-options" : "*"});
//
